...
public class AudioRecorder: NSObject, AVAudioRecorderDelegate{
    ...
    public func getDecibel(_ resolve: RCTPromiseResolveBlock, rejecter reject: @escaping RCTPromiseRejectBlock) -> Void {
        guard let audioRecorder = self.audioRecorder, audioRecorder.isRecording else {
            reject(Constants.audioWaveforms, "No active audio recording", nil)
            return
        }
        let decibelLevel = getDecibelLevel()
        resolve(decibelLevel)
