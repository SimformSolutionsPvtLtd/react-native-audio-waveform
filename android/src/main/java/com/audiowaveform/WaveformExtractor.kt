...
class WaveformExtractor(
    ...
    private fun getFormat(path: String): MediaFormat? {
        Log.d(Constants.LOG_TAG, "Starting get Format: $path- path from RN")
        val mediaExtractor = MediaExtractor()
        this.extractor = mediaExtractor
        val uri = Uri.parse(path)
        mediaExtractor.setDataSource(this.reactApplicationContext, uri, null)
        val trackCount = mediaExtractor.trackCount
        Log.d(Constants.LOG_TAG, "Track Count: $trackCount")
        repeat(trackCount) {
            ...
            decoder = MediaCodec.createDecoderByType(mime).also {
                ...
                it.setCallback(object : MediaCodec.Callback() {
                    ...
                    override fun onOutputFormatChanged(codec: MediaCodec, format: MediaFormat) {
                        ...
                        perSamplePoints = (totalSamples / expectedPoints!!).toLong()
                    }
                    ...
                    override fun onOutputBufferAvailable(
                        ...
                        if (info.isEof()) {
                            ...
                            Log.d(Constants.LOG_TAG, "Auto stop invoked")
                        }
                    }
                })
                it.start()
    ...
    private fun rms(value: Float) {
        try {
            if (sampleCount == perSamplePoints) {
                currentProgress++
                progress = (currentProgress / expectedPoints!!).toFloat()
